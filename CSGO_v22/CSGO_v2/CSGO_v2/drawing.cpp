...
void Render::OutLinedRect(float x, float y, float w, float h, float Thickness, ImColor color)
{
	ImGui::GetBackgroundDrawList()->AddRect(ImVec2(x, y), ImVec2(x + w, y + h), color, 0, ImDrawListFlags_None, Thickness);
}

void Render::FilledRect(float x, float y, float w, float h, ImColor color)
{
...
{
	ImColor Shade;
	Shade.Value = ImVec4(color.Value.x * 0.25f, color.Value.y * 0.25f, color.Value.z * 0.25f, color.Value.w * 0.75f);
